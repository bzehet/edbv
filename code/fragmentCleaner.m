...
while (index<=size(input,1))
    ...
    if (rows(index,1)~=0)
        %if bool is false, this row is first row of a cluster of pixels
        if (bool == false)
            bool = true;
            firstIndex = index;
        end
...
lastIndex = 0;
index = indexMaximum;
%calculates the last row index of the maximum cluster
while (index <size(rows,1))
    if (rows(index,1)==0)
        break;
    end
    lastIndex = index;
